...
class Capture extends Writable {
	...
	// Name
	#name;
	get name() { return `${this.constructor.name}<${this.#name}>`};
	/**
	...
	constructor(condition, timeout) {
		...
		if (timeout !== undefined)
			this.#timeout = setTimeout(this.abort.bind(this), timeout);
	}
	// Abort
	abort() {
		if (!this.#close()) {
			logger.error(`Closing already closed ${this.name}`);
			return false;
		}
		// Log this event
		logger.warn(`Aborting ${this.name}`);
		// Reject the promise with captured stack
		this.#reject(this.#stack);
		return true;
	}
...
export default class CaptureStream extends EndlessWritable {
	...
	clear() {
		[...this.#queue].forEach(capture => capture.abort())
	}
	/**
	...
	capture(cond, timeout = undefined) {
		...
		return capture.on('close', () => {
			logger.verbose(`Removing ${capture.name}`);
