...
export default class Driver extends Lockable {
	...
	async #launch() {
		...
		const
			...
			args = DRIVER_ARGS ?? [];
		// Create the process
		const process = this.#process = spawn(this.#driverPath, args, { stdio: 'pipe' });
		// Connect pipes
		process.stdout.pipe(this.#stream);
		process.stderr.on('data', handleDriverMessage);
		...
		process.on('exit', code => {
			logger.info(`Driver process (pid ${process.pid}) exits with code ${code}`);
	...
	async restart() {
		const self = this;
		return await this.lockExec(async () => {
